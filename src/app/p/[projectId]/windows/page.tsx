...
interface FocusWindow {
  id: string; name: string; startDate: string; endDate: string; isActive: boolean; createdAt: string
  focusOutcomes: { outcome: FocusOutcomeInner }[]
  _count: { focusOutcomes: number }
}

interface AllOutcome { id: string; title: string; stage: Stage; signalStatus: SignalStatus }

function formatDate(d: string) {
  return new Date(d).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
}

export default function ProjectWindowsPage() {
  ...
  const { data: windows = [], mutate, isLoading } = useSWR<FocusWindow[]>(
    workspaceId ? `/api/windows?workspaceId=${workspaceId}&projectId=${projectId}` : null, fetcher
  )

  const { data: allOutcomes = [] } = useSWR<AllOutcome[]>(
    workspaceId && addOutcomeOpen ? `/api/outcomes?workspaceId=${workspaceId}&projectId=${projectId}` : null, fetcher
  )
  ...
  async function createWindow() {
    ...
    try {
      const res = await fetch('/api/windows', {
        ...
        body: JSON.stringify({ name: name.trim(), workspaceId, projectId, startDate: new Date(startDate).toISOString(), endDate: new Date(endDate).toISOString(), isActive }),
      })
      if (!res.ok) { const d = await res.json(); showToast(d.error ?? 'Failed'); return }
      mutate(); setNewOpen(false); setName(''); setStartDate(''); setEndDate('')
      showToast('Focus Window created')
    } finally { setSaving(false) }
  }

  async function addOutcome(outcomeId: string) {
    if (!selectedWindow) return
    const res = await fetch(`/api/windows/${selectedWindow.id}/outcomes`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ outcomeId }) })
    if (res.ok) { mutate(); showToast('Outcome added to window') }
  }
    ...
    <div style={{ flex: 1, overflowY: 'auto', background: '#040e17' }}>
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '28px 24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: '#6b8fa8' }}>{projectName} · Time-boxed focus periods for this project</p>
          </div>
          <button onClick={() => setNewOpen(true)} style={{ padding: '9px 18px', background: 'rgba(45,212,191,0.12)', color: '#2dd4bf', border: '1px solid rgba(45,212,191,0.3)', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' }}>
            + New Focus Window
          </button>
        </div>
        ...
        {isLoading ? (
          ...
          <div style={{ textAlign: 'center', padding: '64px 0' }}>
            ...
            <h3 style={{ color: '#a8ccd8', fontSize: 16, fontWeight: 600, marginBottom: 6 }}>No Focus Windows yet</h3>
            <p style={{ color: '#3a5a6e', fontSize: 13, maxWidth: 360, margin: '0 auto 20px' }}>Define what your team is flowing toward this cycle — no ceremonies, just clarity.</p>
            <button onClick={() => setNewOpen(true)} style={{ padding: '9px 20px', background: 'rgba(45,212,191,0.12)', color: '#2dd4bf', border: '1px solid rgba(45,212,191,0.3)', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>+ New Focus Window</button>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {windows.map(w => {
              ...
              return (
                <div key={w.id} style={{ background: '#061c2e', border: `1px solid ${isSelected ? 'rgba(45,212,191,0.35)' : w.isActive ? 'rgba(45,212,191,0.2)' : 'rgba(45,212,191,0.08)'}`, borderRadius: 12, overflow: 'hidden', transition: 'border-color 0.2s' }}>
                  <div style={{ padding: '16px 20px', cursor: 'pointer' }} onClick={() => setSelectedWindow(isSelected ? null : w)}>
                    ...
                    <div style={{ fontSize: 11, color: '#6b8fa8', marginTop: 4 }}>{Math.round(pct)}% flow rate</div>
                  </div>

                  {isSelected && (
                    <div style={{ borderTop: '1px solid rgba(45,212,191,0.1)', padding: '16px 20px', background: '#040e17' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                        <span style={{ fontSize: 12, fontWeight: 600, color: '#6b8fa8', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Active outcomes in this flow cycle</span>
                      ...
                      {w.focusOutcomes.length === 0 ? (
                        <p style={{ fontSize: 13, color: '#3a5a6e', fontStyle: 'italic' }}>No outcomes in this flow cycle yet</p>
                      ) : (
      ...
      {newOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(4,14,23,0.88)', zIndex: 400, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={e => { if (e.target === e.currentTarget) setNewOpen(false) }}>
          <div style={{ background: '#061c2e', border: '1px solid rgba(45,212,191,0.25)', borderRadius: 14, padding: '28px', width: 420, maxWidth: '95vw' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
              <h3 style={{ fontSize: 17, fontWeight: 700, color: '#e8f4f8' }}>New Focus Window</h3>
              <button onClick={() => setNewOpen(false)} style={{ background: 'none', border: 'none', color: '#6b8fa8', fontSize: 20, cursor: 'pointer', lineHeight: 1 }}>×</button>
            </div>
            <p style={{ fontSize: 12, color: '#6b8fa8', marginBottom: 20, lineHeight: 1.5 }}>A Focus Window defines what outcomes your team is actively flowing toward.</p>
            <div style={{ marginBottom: 14 }}><label style={labelSt}>Window Name</label><input style={inputSt} value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Q1 Flow, March Focus, MVP Push" autoFocus /></div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
              <div><label style={labelSt}>Flow Start</label><input type="date" style={inputSt} value={startDate} onChange={e => setStartDate(e.target.value)} /></div>
              <div><label style={labelSt}>Flow End</label><input type="date" style={inputSt} value={endDate} onChange={e => setEndDate(e.target.value)} /></div>
            </div>
            <div style={{ marginBottom: 24 }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                ...
                <span style={{ fontSize: 13, color: '#a8ccd8' }}>Set as active flow window</span>
              </label>
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={createWindow} disabled={!name.trim() || !startDate || !endDate || saving} style={{ flex: 2, padding: '10px 0', background: name.trim() ? 'rgba(45,212,191,0.15)' : 'rgba(45,212,191,0.05)', color: name.trim() ? '#2dd4bf' : '#3a5a6e', border: `1px solid ${name.trim() ? 'rgba(45,212,191,0.4)' : 'rgba(45,212,191,0.1)'}`, borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: name.trim() ? 'pointer' : 'not-allowed' }}>{saving ? 'Creating…' : 'Create Focus Window'}</button>
            </div>
          </div>
        </div>
      )}

      {/* Add Outcome to Window Modal */}
      {addOutcomeOpen && selectedWindow && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(4,14,23,0.88)', zIndex: 400, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={e => { if (e.target === e.currentTarget) setAddOutcomeOpen(false) }}>
          <div style={{ background: '#061c2e', border: '1px solid rgba(45,212,191,0.25)', borderRadius: 14, padding: '24px', width: 420, maxWidth: '95vw', maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
              <h3 style={{ fontSize: 16, fontWeight: 700, color: '#e8f4f8' }}>Add Outcome to Flow Cycle</h3>
              <button onClick={() => setAddOutcomeOpen(false)} style={{ background: 'none', border: 'none', color: '#6b8fa8', fontSize: 20, cursor: 'pointer', lineHeight: 1 }}>×</button>
            </div>
            <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6 }}>
              {allOutcomes.filter(o => !selectedWindow.focusOutcomes.some(fo => fo.outcome.id === o.id)).map(o => (
              ...
              {allOutcomes.filter(o => !selectedWindow.focusOutcomes.some(fo => fo.outcome.id === o.id)).length === 0 && (
                <p style={{ fontSize: 13, color: '#3a5a6e', textAlign: 'center', padding: '20px 0', fontStyle: 'italic' }}>All outcomes are already in this flow cycle.</p>
              )}
            </div>
          </div>
        </div>
      ...
      {toast && <div style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 9999, padding: '10px 16px', borderRadius: 8, background: '#061c2e', border: '1px solid rgba(45,212,191,0.4)', color: '#2dd4bf', fontSize: 13, fontWeight: 600, boxShadow: '0 4px 20px rgba(0,0,0,0.4)' }}>✓ {toast}</div>}
    </div>
  )
}

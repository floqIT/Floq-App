...
export default function WindowsPage() {
  ...
  const inputSt: React.CSSProperties = {
    ...
    padding: '8px 12px', color: '#e8f4f8', fontSize: 13, outline: 'none', width: '100%', boxSizing: 'border-box',
  }
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', background: '#040e17' }}>
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '28px 24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: '#6b8fa8' }}>Time-boxed focus periods — define what your team is flowing toward each cycle.</p>
          </div>
          <button
            onClick={() => setNewOpen(true)}
            style={{ padding: '9px 18px', background: 'rgba(45,212,191,0.12)', color: '#2dd4bf', border: '1px solid rgba(45,212,191,0.3)', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' }}
          >
            + New Focus Window
          </button>
        </div>
        ...
        {isLoading ? (
          ...
          <div style={{ textAlign: 'center', padding: '64px 0' }}>
            ...
            <p style={{ color: '#3a5a6e', fontSize: 13, maxWidth: 360, margin: '0 auto 20px' }}>
              Define what your team is flowing toward this cycle — no ceremonies, just clarity.
            </p>
            <button
              onClick={() => setNewOpen(true)}
              style={{ padding: '9px 20px', background: 'rgba(45,212,191,0.12)', color: '#2dd4bf', border: '1px solid rgba(45,212,191,0.3)', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}
            >
              + New Focus Window
            </button>
          </div>
                ...
                <div key={w.id} style={{ background: '#061c2e', border: `1px solid ${isSelected ? 'rgba(45,212,191,0.35)' : w.isActive ? 'rgba(45,212,191,0.2)' : 'rgba(45,212,191,0.08)'}`, borderRadius: 12, overflow: 'hidden', transition: 'border-color 0.2s' }}>
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                      ...
                      {w.isActive && (
                        <span style={{ fontSize: 10, fontWeight: 800, background: 'rgba(34,197,94,0.15)', color: '#22c55e', border: '1px solid rgba(34,197,94,0.3)', borderRadius: 20, padding: '2px 8px', letterSpacing: '0.08em' }}>ACTIVE</span>
                      )}
                    </div>
                  ...
                  {isSelected && (
                    <div style={{ borderTop: '1px solid rgba(45,212,191,0.1)', padding: '16px 20px', background: '#040e17' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                        <span style={{ fontSize: 12, fontWeight: 600, color: '#6b8fa8', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                          Active outcomes in this flow cycle
                        </span>
                        <button
                          onClick={() => setAddOutcomeOpen(true)}
                          style={{ padding: '5px 12px', background: 'rgba(45,212,191,0.1)', border: '1px solid rgba(45,212,191,0.3)', color: '#2dd4bf', borderRadius: 7, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}
                        >
                          + Add Outcome
                        </button>
                      </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                          {w.focusOutcomes.map(fo => (
                            <div key={fo.outcome.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: '#061c2e', borderRadius: 8, border: '1px solid rgba(45,212,191,0.08)' }}>
                              ...
                              <button
                                onClick={() => removeOutcome(fo.outcome.id)}
                                style={{ background: 'none', border: 'none', color: '#3a5a6e', cursor: 'pointer', fontSize: 14, lineHeight: 1, padding: 2 }}
      ...
      {newOpen && (
        ...
        >
          <div style={{ background: '#061c2e', border: '1px solid rgba(45,212,191,0.25)', borderRadius: 14, padding: '28px', width: 420, maxWidth: '95vw' }}>
            ...
            <p style={{ fontSize: 12, color: '#6b8fa8', marginBottom: 20, lineHeight: 1.5 }}>
              A Focus Window defines what outcomes your team is actively flowing toward. No ceremonies. Just clarity.
            </p>

            <div style={{ marginBottom: 14 }}>
              <label style={labelSt}>Window Name</label>
              <input
                style={inputSt}
                value={name}
                onChange={e => setName(e.target.value)}
                placeholder="e.g. Q1 Flow, March Focus, Launch Cycle 1, MVP Push"
                autoFocus
              />
            </div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button
                ...
                style={{ flex: 2, padding: '10px 0', background: name.trim() ? 'rgba(45,212,191,0.15)' : 'rgba(45,212,191,0.05)', color: name.trim() ? '#2dd4bf' : '#3a5a6e', border: `1px solid ${name.trim() ? 'rgba(45,212,191,0.4)' : 'rgba(45,212,191,0.1)'}`, borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: name.trim() ? 'pointer' : 'not-allowed' }}
              >
                {saving ? 'Creating…' : 'Create Focus Window'}
              </button>
            </div>
          </div>
        </div>
      ...
      {addOutcomeOpen && selectedWindow && (
        <div
          style={{ position: 'fixed', inset: 0, background: 'rgba(4,14,23,0.88)', zIndex: 400, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          onClick={e => { if (e.target === e.currentTarget) setAddOutcomeOpen(false) }}
        >
            ...
            <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6 }}>
              {allOutcomes
                ...
                .map(o => (
                  <button
                    key={o.id}
                    onClick={() => { addOutcome(o.id); setAddOutcomeOpen(false) }}
                    style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: '#040e17', border: '1px solid rgba(45,212,191,0.1)', borderRadius: 8, cursor: 'pointer', textAlign: 'left', transition: 'border-color 0.15s' }}

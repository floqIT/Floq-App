...
export default async function MetricsPage() {
  ...
  const stageCounts: Record<Stage, number> = { IDEATE: 0, IDENTIFY: 0, SHAPE: 0, BUILD: 0, QA: 0, SHIP: 0, MEASURE: 0, DELIVER: 0, PIVOT: 0 }
  ...
  // Cycle time: avg days from createdAt to deliveredAt for delivered outcomes
  ...
  // Top 5 by impact
  const top5 = [...outcomes].sort((a, b) => b.impactScore - a.impactScore || b.updatedAt.getTime() - a.updatedAt.getTime()).slice(0, 5)
    ...
    <div style={{ flex: 1, overflowY: 'auto', background: '#040e17' }}>
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '28px 24px' }}>
        <div style={{ marginBottom: 28 }}>
          ...
          <p style={{ fontSize: 13, color: '#6b8fa8' }}>Outcome health and delivery insights</p>
        </div>
        ...
        {total === 0 ? (
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            ...
            <p style={{ color: '#3a5a6e', fontSize: 13 }}>Create some outcomes on the board to start seeing metrics.</p>
          </div>
        ) : (
          <>
            {/* KPI row */}
                    ...
                    return (
                      <div key={stage}>
                        ...
                        <div style={{ height: 8, background: 'rgba(255,255,255,0.05)', borderRadius: 4, overflow: 'hidden' }}>
                          <div style={{ height: '100%', width: `${pct}%`, background: STAGE_COLORS[stage], borderRadius: 4, transition: 'width 0.7s ease', opacity: count > 0 ? 1 : 0.3 }} />
                        </div>
                      </div>
                    )
                  })}
              ...
              <div style={{ background: '#061c2e', border: '1px solid rgba(45,212,191,0.1)', borderRadius: 12, padding: '20px 22px' }}>
                ...
                <div style={{ display: 'flex', height: 20, borderRadius: 10, overflow: 'hidden', marginBottom: 16 }}>
                  {signalList.map(s => {
                    ...
                    return pct > 0 ? (
                      <div key={s.key} title={`${s.label}: ${signalCounts[s.key]}`} style={{ width: `${pct}%`, background: s.color, transition: 'width 0.5s ease' }} />
                    ) : null
                  })}
                </div>
                ...
                {top5.map((o, i) => (
                  <div key={o.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: '#040e17', border: '1px solid rgba(45,212,191,0.08)', borderRadius: 8 }}>
                    ...
                    <div style={{ display: 'flex', gap: 3 }}>
                      {[1,2,3,4,5].map(d => (
                        <span key={d} style={{ width: 8, height: 8, borderRadius: '50%', background: d <= o.impactScore ? '#2dd4bf' : 'rgba(45,212,191,0.15)' }} />
                      ))}
                    </div>
